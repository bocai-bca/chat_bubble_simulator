...
func _base_process(__delta: float) -> void:
	...
	if (_show_corner): #如果本气泡应显示气泡角
		...
		__corner_radius = lerpf(CBSConfig.bubble_corner_min_radius, CBSConfig.bubble_unit_pixel * CBSConfig.bubble_capsule_thickness_multiplier, ease(_tf_corner_timer / CBSConfig.bubble_corner_animation_time, CBSConfig.bubble_transform_ease_curve))
	...
	# /气泡角变换计算
	#/网格形状的变换计算
	n_text.set_position(n_quad.position + n_text.size / -2.0) #将文本居中于气泡中心
...
func _old_build_bubble() -> void: #直接成品气泡构建函数，将气泡各项状态调整到与当前文本、发送方、配置参数适配的状态，只在直接创建成品气泡时调用
	...
	#/气泡构成组织
	#气泡缩放
	(n_up_capsule.mesh as CapsuleMesh).radius = CBSConfig.bubble_unit_pixel * CBSConfig.bubble_capsule_thickness_multiplier
	#/气泡缩放
