...
func _physics_process(__delta: float) -> void:
	...
	position = _tf_pos_from.lerp(_tf_pos_to, smoothstep(0.0, CBSConfig.bubble_fadein_time, _tf_pos_timer)) #平滑阶梯，二选一
	...
	#网格形状的变换计算
	var __mesh_transform_process: float = smoothstep(0.0, CBSConfig.bubble_fadein_time, _tf_bubble_time) #平滑阶梯，二选一
	#var __mesh_transform_process: float = clampf(_tf_bubble_time / CBSConfig.bubble_fadein_time, 0.0, 1.0) #线性，二选一
	(n_up_capsule.mesh as CapsuleMesh).height = _tf_bubble_x_from + (_tf_bubble_x_to - _tf_bubble_x_from) * __mesh_transform_process
	(n_quad.mesh as QuadMesh).size.x = _tf_bubble_x_from + (_tf_bubble_x_to - _tf_bubble_x_from) * __mesh_transform_process
...
func _build_bubble() -> void: #直接成品气泡构建函数，将气泡各项状态调整到与当前文本、发送方、配置参数适配的状态，只在直接创建成品气泡时调用
	#气泡构成组织
...
func _fit_position() -> void: #根据气泡渲染的形状适配坐标变换的起始位置和目标位置，跟随气泡构建函数一起使用，只能调用一次。适用于输入气泡和直接成品的气泡
	#设定气泡位置
	...
	match (_send_from_right):
		false: #左侧
			CBS.last_bubble_side = 1
		true: #右侧
			CBS.last_bubble_side = 2
	#/设定气泡位置
